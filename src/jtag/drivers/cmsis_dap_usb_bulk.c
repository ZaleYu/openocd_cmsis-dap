/***************************************************************************
 *   Copyright (C) 2018 by Mickael Thomas                                  *
 *   mickael9@gmail.com                                                    *
 *                                                                         *
	...
	for (int i = 0; i < num_devices; i++) {
		...
		for (int config = 0; config < dev_desc.bNumConfigurations; config++) {
			...
			for (int interface = 0; interface < config_desc->bNumInterfaces; interface++) {
				 ...
				 *  - Endpoint 1: Bulk Out ¡V used for commands received from host PC.
				 *  - Endpoint 2: Bulk In ¡V used for responses send to host PC.
				 *  - Endpoint 3: Bulk In (optional) ¡V used for streaming SWO trace (if enabled with SWO_STREAM).
				 */
